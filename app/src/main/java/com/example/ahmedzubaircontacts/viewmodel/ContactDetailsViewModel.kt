    ...
    fun getFullContact(personId: Long) {
        launch {
            personLiveData.value =
                ContactDatabase(getApplication()).personDAO().getPersonById(personId)
            phonesLiveData.value = ArrayList(
                ContactDatabase(getApplication()).phoneDAO().findAllPhonesByPersonId(personId)
            )
            emailsLiveData.value = ArrayList(
                ContactDatabase(getApplication()).emailDAO().findAllEmailsByPersonId(personId)
            )
            addressesLiveData.value = ArrayList(
                ContactDatabase(getApplication()).addressDAO().findAllAddressesByPersonId(personId)
            )
        }
    }
